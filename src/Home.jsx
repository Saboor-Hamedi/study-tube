  ...
  const launchPoints = [
    {
      ...
      description:
        "Structural auditing and neural origin detection for academic drafts.",
      icon: Cpu,
      color: "text-accent",
      bg: "bg-accent/10",
    ...
    },
    {
      ...
      description:
        "Extract scholarly insights and neural transcripts from global video.",
      icon: Search,
      color: "text-red-500",
      bg: "bg-red-500/10",
    ...
    },
    {
      ...
      description:
        "Audit documents for synthetic predictability and algorithmic patterns.",
      icon: Shield,
    ...
    },
    {
      ...
      description:
        "Manage your persistent library of neural nodes and archived audits.",
      icon: Library,
      ...
      <div className="flex-1 overflow-y-auto px-12 py-12 flex flex-col items-center relative">
        ...
        >
          ...
          <p className="text-[12px] text-muted font-medium leading-relaxed max-w-md mx-auto opacity-70">
            High-density environment for neural linguistic auditing and
            structural origin detection.
          </p>
        </motion.div>
        ...
        >
          {launchPoints.map((point) => (
            ...
            >
              <div
                className={`absolute inset-0 bg-gradient-to-br ${point.accent} opacity-0 group-hover:opacity-100 transition-opacity rounded-[12px] blur-[2px]`}
              />
              <div
              ...
              >
                <div>
                  <div className="flex items-start justify-between mb-6">
                    <div
                      className={`p-3 rounded-[10px] ${point.bg} group-hover:scale-105 transition-transform`}
                    >
                      <point.icon className={`h-5 w-5 ${point.color}`} />
                    </div>
                    <div
                      className={`p-1.5 rounded-full ${point.bg} opacity-0 group-hover:opacity-100 group-hover:bg-accent group-hover:text-white transition-all`}
                    >
                  ...
                  <div className="space-y-0.5">
                    <p
                      className={`text-[9px] font-black uppercase tracking-widest ${point.color} opacity-80`}
                    >
                      {point.subtitle}
                    </p>
                    <h3 className="text-[18px] font-black text-text uppercase tracking-tight">
                      {point.title}
                    </h3>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex -space-x-1.5">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="h-5 w-5 rounded-full border border-surface-2 bg-surface-3 flex items-center justify-center overflow-hidden"
                        >
        ...
        >
          <div className="space-y-0.5">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-[20px] font-black text-text tracking-tighter">
                {vocabCount}
              </span>
      ...
      <div className="h-[72px] border-t border-border/5 px-8 flex items-center justify-between bg-surface-2/30 backdrop-blur-md">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] font-black text-text/60 uppercase tracking-widest">
              Neural Network: Connected
            </span>
          </div>
          <div className="h-3 w-px bg-border/10" />
          <span className="text-[8px] font-black text-text/40 uppercase tracking-widest leading-none">
            System Load: 12%
          </span>
        </div>
        <div className="flex items-center gap-2">
