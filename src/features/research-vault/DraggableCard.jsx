...
export const DroppableFolder = memo(
  ({ id, active, onClick, children, onDelete, isCollapsed = false }) => {
    ...
    return (
      ...
      >
        ...
        >
          <div
            className={`transition-colors duration-200 shrink-0 flex items-center justify-center ${isOver ? "text-text" : active ? "text-text" : "opacity-40 group-hover:opacity-100"}`}
          >
            {id === "all" ? (
          ...
          {!isCollapsed && (
            <span
              className={`text-[10px] uppercase font-black tracking-[0.2em] truncate flex-1 ${isOver ? "text-text" : ""}`}
            >
              {children}
            </span>
          )}
