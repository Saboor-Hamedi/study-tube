...
const InsightCaptureModal = ({ isOpen, onClose, onInsert, showToast, api }) => {
  ...
  const handleSave = async () => {
    if (!title.trim())
      return showToast("Subject Designation Required", "error");
    setIsSaving(true);
  ...
  return (
    <AnimatePresence>
      <div
        className={`fixed inset-0 z-[1000] flex items-center justify-center transition-all duration-500 ${isMaximized ? "p-0" : "p-0 lg:p-8"}`}
      >
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <div
            className={`flex-1 bg-surface flex flex-col overflow-hidden ${isMaximized ? "rounded-none" : "rounded-l-[5px]"}`}
          >
