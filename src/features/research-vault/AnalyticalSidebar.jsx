...
const AnalyticalSidebar = ({
  ...
  return (
    <aside
      className={`shrink-0 border-l border-border bg-surface-2 flex flex-col relative z-50 ${isModal ? "w-[280px]" : "w-[300px] pt-8 overflow-y-auto"} ${isModal && !isMaximized ? "rounded-r-[5px]" : ""} scrollbar-thin`}
    >
      {isModal && (
        <ModalCommandHeader
      ...
      >
        ...
        {!isCapture && (
          <>
            {/* Forensic Diagnostics (If Archive Node) */}
            {item?.diagnostics && (
              <div className="space-y-4">
                <p className="text-[9px] font-black uppercase tracking-[0.3em] text-muted px-1 flex items-center gap-2 opacity-30">
                  <Brain className="h-3 w-3" /> Forensic Report
                </p>
                <div className="p-4 bg-surface-3 border border-border rounded-[5px] space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-text/40 uppercase tracking-widest">
                      IELTS Score
                    </span>
                    <span className="text-[14px] font-black text-accent bg-accent/5 px-2 py-0.5 rounded-[3px] border border-accent/20">
                      BAND{" "}
                      {item.diagnostics.ielts ||
                        item.diagnostics.ieltsBand ||
                        "N/A"}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { label: "Grammar", value: item.diagnostics.grammar },
                      { label: "Academic", value: item.diagnostics.academic },
                      {
                        label: "Rigor",
                        value:
                          item.diagnostics.writing || item.diagnostics.rigor,
                      },
                      { label: "Density", value: item.diagnostics.index },
                    ].map((stat) => (
                      <div
                        key={stat.label}
                        className="p-2 bg-background/40 border border-border/5 rounded-[3px] flex flex-col gap-0.5"
                      >
                        <span className="text-[7px] font-black text-muted/40 uppercase tracking-tighter">
                          {stat.label}
                        </span>
                        <span className="text-[11px] font-black text-text tracking-tight">
                          {stat.value || 0}%
                        </span>
                      </div>
                    ))}
                ...
                {isEditing ? (
                  <div className="space-y-1.5">
                    <button
                      ...
                      className="w-full py-2.5 bg-accent text-white text-[9px] font-black uppercase tracking-widest hover:brightness-110 active:scale-[0.98] transition-all shadow-lg shadow-accent/20 rounded-[5px]"
                    >
                      Commit Changes
                    </button>
                    <button
                      onClick={() => setIsEditing(false)}
                      className="w-full py-2.5 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white text-[9px] font-black uppercase tracking-widest transition-all rounded-[5px] border border-red-500/20"
                    >
                      Abort Edit
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="grid grid-cols-2 gap-1.5">
                      <button
                        ...
                        className="py-2.5 bg-accent text-white text-[9px] font-black uppercase tracking-widest hover:brightness-110 transition-all flex items-center justify-center gap-2 rounded-[5px]"
                      >
                ...
                >
                  <div className="p-1.5 bg-accent/10 rounded-[3px]">
                    {isSummarizing ? (
                      ...
                      <Sparkles className="h-3 w-3 text-accent transition-colors group-hover:text-white" />
                    )}
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-text uppercase tracking-widest group-hover:text-accent transition-colors">
                      Synthesis
                    </p>
                    <p className="text-[8px] text-muted uppercase tracking-tight">
                      Core Pillars
                    </p>
                  </div>
                </button>
                  ...
                  >
                    <div className="p-1.5 bg-muted/10 rounded-[3px]">
                      {isHighlighting ? (
                        ...
                        <ListChecks className="h-3 w-3 text-text" />
                      )}
                    </div>
                    <div>
