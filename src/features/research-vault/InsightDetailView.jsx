...
const neuralComponents = {
  ol: ({ node, ...props }) => (
    <ol
      className="list-decimal pl-10 space-y-4 my-8 text-accent marker:text-accent marker:font-black"
      {...props}
    />
  ),
  ul: ({ node, ...props }) => (
    <ul
      className="list-disc pl-10 space-y-4 my-8 text-accent marker:text-accent"
      {...props}
    />
  ),
  li: ({ node, ...props }) => (
    <li className="text-text leading-[1.8] pl-2 font-extralight" {...props} />
  ),
  p: ({ node, ...props }) => (
    <p
      className="mb-6 last:mb-0 text-text leading-[1.8] font-extralight"
      {...props}
    />
  ),
  strong: ({ node, ...props }) => (
    <strong
      className="text-accent font-bold tracking-tight px-0.5 border-b border-accent/20 drop-shadow-[0_0_2px_rgba(var(--accent-rgb),0.4)]"
      {...props}
    />
  ),
...
const InsightDetailView = ({
    ...
    const hydrate = async () => {
      if (item) {
        ...
        if (!item.definition && (item.id || item.date)) {
          console.log(
            "[NEURAL WORKSPACE] Shallow node detected. Synchronizing with archive...",
          );
          try {
  ...
  // Auto-sync with Copilot context (with 500ms debounce for performance)
  useEffect(() => {
    const timer = setTimeout(() => {
      if (onOpenCopilot) {
        ...
        const itemKey = `${item?.id || item?.date}-${currentText}-${currentDef}`;
  ...
  }, [item, isEditing, editVal, titleEditVal, onOpenCopilot]);
  ...
  return (
    ...
    >
      ...
      <div className="h-[56px] shrink-0 flex items-center justify-between px-5 border-b border-border/20 bg-surface">
        <h2 className="text-[13px] font-black text-text tracking-tight truncate pr-4">
          {item.text}
        </h2>
