                            ...
                            >
                              <div className="flex items-center gap-2.5">
                                ...
                                <div className="min-w-0">
                                 <h4 className="text-[10px] font-black text-text truncate">
                                  {item.text}
                                </h4>
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="text-[8px] text-muted font-bold uppercase tracking-widest shrink-0">{new Date(item.date).toLocaleDateString()}</span>
                                  {(item.band || item.metadata?.band || item.diagnostics?.ielts || item.metadata?.diagnostics?.ielts) && (
                                    <span className="flex items-center gap-1 text-[8px] font-black text-accent bg-accent/10 px-1 py-0.5 rounded uppercase tracking-widest shrink-0">
                                      <Award className="h-2 w-2" /> Band {item.band || item.metadata?.band || item.diagnostics?.ielts || item.metadata?.diagnostics?.ielts}
                                    </span>
                                  )}
                                </div>
                              </div>
