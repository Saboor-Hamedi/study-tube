...
const GrammarBody = ({ content = "", onNavigate }) => {
  // Extract title and filter out the TOC header if it exists
  const { title, cleanContent } = useMemo(() => {
    let raw = content;
    
    // Remove the specific TOC title if it exists
    raw = raw.replace(/# 📚 Grammar Archive – Table of Contents\n?/, "");
    
    const titleMatch = raw.match(/^#\s+(.+)/);
    const extractedTitle = titleMatch ? titleMatch[1] : "Writella Rule Node";
  ...
  const stats = useMemo(() => {
    ...
    const complexity = words > 1000 ? "High" : words > 500 ? "Medium" : "Standard";
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-surface-3 overflow-hidden relative selection:bg-accent/10">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin relative z-10 flex flex-col">
        ...
        {/* Diagnostic Header (Stuck/Sticky-feel Top) */}
          ...
          <div className="w-full max-w-5xl relative">
            ...
            <div className="flex flex-wrap gap-6 md:gap-10 select-text cursor-text">
              <div className="flex flex-col gap-1">
                <span className="text-[8px] font-black text-muted/30 uppercase tracking-[0.25em]">Reading Time</span>
                <div className="flex items-center gap-2">
                  <Clock className="h-3.5 w-3.5 text-accent/40" />
                  <span className="text-[10px] font-black tabular-nums text-text/80">{stats.readingTime} MIN</span>
                </div>
              </div>
              
              <div className="flex flex-col gap-1">
                <span className="text-[8px] font-black text-muted/30 uppercase tracking-[0.25em]">Complexity</span>
                <div className="flex items-center gap-2">
                  <Zap className="h-3.5 w-3.5 text-accent/40" />
                  <span className="text-[10px] font-black uppercase text-text/80">{stats.complexity}</span>
                </div>
              </div>

              <div className="flex flex-col gap-1 sm:ml-auto min-w-[120px]">
                <span className="text-[8px] font-black text-muted/30 uppercase tracking-[0.25em]">Node Integrity</span>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[9px] font-mono font-black text-accent/60">98%</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Writella Content Body (White Background, Full Width Feel) */}
          ...
          <div className="w-full max-w-6xl px-6 sm:p-8 md:p-10 py-8">
                ...
                components={{
                  ...
                  h2: ({ children }) => (
                    <h2 className="select-text cursor-pointer mb-4 mt-12 text-lg font-black flex items-center gap-3 text-text/90 uppercase tracking-tight">
                      <div className="w-1 h-5 bg-accent/40 rounded-full shrink-0" />
                      {children}
                    </h2>
                  ),
                  h3: ({ children }) => (
                  ...
                  ul: ({ children }) => (
                    <ul className="mb-8 list-disc list-inside space-y-1">{children}</ul>
                  ),
                  ol: ({ children }) => (
                    <ol className="mb-8 list-decimal list-inside space-y-1">{children}</ol>
                  ),
                  strong: ({ children }) => (
                    <strong className="font-bold text-accent/80 select-text">{children}</strong>
                  ),
                  em: ({ children }) => (
                    <em className="italic opacity-80 select-text">{children}</em>
                  ),
                  code: ({ children }) => (
            ...
            <div className="mt-16 pt-8 border-t border-surface-3 flex justify-center">
              <p className="text-[9px] text-muted/20 font-black uppercase tracking-[0.4em] italic select-text">
                Specification Complete — Writella Intelligence Archive
              </p>
            </div>
          </div>
        </div>
