...
export default function Sidebar({ 
  ...
  return (
    <div className="w-56 border-r border-border bg-surface-2 flex flex-col overflow-hidden transition-colors duration-500">
          ...
          <div className="flex items-center justify-between px-2">
            ...
            {setIsCreatingCollection && (
              <button 
                ...
                className="p-1 h-5 w-5 flex items-center justify-center bg-surface-3 hover:bg-accent hover:text-white text-muted transition-all duration-300 rounded-[5px]"
              >
          ...
          <div className="space-y-1">
            {collections.map(c => (
              <div key={c} className="group/folder relative">
                {renamingId === c ? (
                  <div className="mx-1 p-1 bg-surface-3 border border-accent/20 flex items-center gap-1 overflow-hidden rounded-[5px]">
                    <input 
                      autoFocus
                      value={renamingValue}
                    ...
                    />
                    <button onClick={submitRename} className="p-1 text-accent hover:bg-accent/10 shrink-0 rounded-[5px]">
                  ...
                  <>
                    ...
                    <div className="absolute right-8 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover/folder:opacity-100 transition-opacity">
                       <button 
                        ...
                        className="p-1 bg-surface-3 hover:bg-accent text-muted hover:text-white transition-all rounded-[5px]"
                       >
             ...
             {isCreatingCollection && (
               <div className="mx-1 mt-2 p-2 border border-accent/20 bg-accent/[0.02] animate-in fade-in zoom-in-95 duration-200 rounded-[5px]">
                 <input 
                   autoFocus
                 ...
                 <div className="flex gap-1 mt-2">
                   ...
                   <button onClick={() => setIsCreatingCollection(false)} className="px-3 py-1 bg-surface-3 text-muted text-[9px] font-black uppercase tracking-widest hover:text-text transition-all rounded-[5px] border border-border">X</button>
                 </div>
               </div>
             )}
          </div>
