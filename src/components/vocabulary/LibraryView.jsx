              ...
              <div className="space-y-3 pr-10">
                 <div className="flex items-center flex-wrap gap-3">
                    {v.text?.split(' ').length > 3 ? (
                      ...
                      <h3 className="text-xl font-black text-white">{v.text}</h3>
                    )}
                    {v.type && v.text?.split(' ').length <= 3 && <span className="px-2 py-0.5 bg-white/5 text-muted text-[10px] font-bold uppercase tracking-widest rounded-full border border-white/5 truncate max-w-[100px] inline-block">{v.type.split(/[.,(]/)[0].trim().substring(0, 15)}</span>}
                    {v.pronunciation && v.text?.split(' ').length <= 3 && <span className="text-[10px] font-mono text-accent bg-accent/10 px-1.5 py-0.5 rounded border border-accent/20">/{v.pronunciation}/</span>}
                 </div>
                 {v.loading ? (
