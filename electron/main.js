...
function atomicWriteJsonSync(filePath, data) {
  ...
  try {
    ...
    */
    ...
    console.log(
      `[SENTRY] Snapshot & Atomic write successful: ${path.basename(filePath)}`,
    );
    return true;
  } catch (err) {
    console.error(
      `[CRITICAL] Atomic write failed: ${path.basename(filePath)}`,
      err,
    );
...
function canonicalize(url) {
  ...
  try {
    if (ytdl.validateURL(s))
      return `https://www.youtube.com/watch?v=${ytdl.getURLVideoID(s)}`;
  } catch {}
  return s;
}
...
async function performCloudSync() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.warn(
        `[CLOUD SYNC] Server rejected sync: ${response.statusText} - ${errorText}`,
      );
      return { success: false, message: response.statusText };
    }
  } catch (err) {
    console.error(
      "[CLOUD SYNC] Neural Bridge connection failure:",
      err.message,
    );
...
async function fetchMetadata(url) {
  ...
  const thumbnail =
    [...(details.thumbnails ?? [])].sort(
      (a, b) => (b.width ?? 0) - (a.width ?? 0),
    )[0]?.url || "";
  const rawTitle = details.title || "";
  const cleanTitle =
    rawTitle === "YouTube Video" || rawTitle === "youtube video"
      ? ""
      : rawTitle;
  ...
  return {
    ...
    qualityOptions: heights.length
      ? buildQualityOptions(heights)
...
async function downloadVideo({
  ...
  const stem = isAudio
    ? safeTitle
    : `${safeTitle}-${Number.isFinite(targetH) ? targetH : "best"}p`;
  ...
  activeDownloads.set(taskId, {
    abort() {
      aborted = true;
      try {
        activeStream1?.destroy();
      } catch {}
      try {
        activeStream2?.destroy();
      } catch {}
      try {
  ...
  if (isAudio) {
    ...
    const audioTmp = path.join(
      savePath,
      `${safeTitle}_a_${taskId.slice(0, 8)}.tmp`,
    );
    ...
    await new Promise((resolve, reject) => {
      ...
      stream.on("data", (chunk) => {
        ...
        if (total > 0) {
          ...
          sendToRenderer(webContents, "download:progress", {
            taskId,
            percent: pct,
  ...
  const videoTmp = path.join(
    savePath,
    `${safeTitle}_v_${taskId.slice(0, 8)}.tmp`,
  );
  const audioTmp = path.join(
    savePath,
    `${safeTitle}_a_${taskId.slice(0, 8)}.tmp`,
  ...
  const totalSize =
    (Number(videoFmt.contentLength) || 0) +
    (Number(audioFmt?.contentLength) || 0);
  let downloaded = 0;
  ...
  await new Promise((resolve, reject) => {
    ...
    stream.on("data", (chunk) => {
      ...
      const pct =
        totalSize > 0
          ? Math.min(55, Math.round((downloaded / totalSize) * 80))
          : 0;
      sendToRenderer(webContents, "download:progress", {
        taskId,
        percent: pct,
  ...
  await new Promise((resolve, reject) => {
    ...
    stream.on("data", (chunk) => {
      ...
      const pct =
        totalSize > 0
          ? Math.min(85, Math.round((downloaded / totalSize) * 80))
          : 60;
      sendToRenderer(webContents, "download:progress", {
        taskId,
        percent: pct,
...
function registerIpcHandlers() {
    ...
    try {
      ...
      ipcMain.handle(channel, async (...args) => {
        try {
          ...
          console.error(`[IPC FAIL] ${channel}:`, e.message);
          throw e;
        }
      });
    } catch (e) {
        ...
        "https://api.deepseek.com/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: `Neural Assistant. Context: ${context}`,
              },
              ...messages,
            ],
            temperature: 0.7,
        ...
        "https://api.deepseek.com/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: `Friendly English Tutor. Context: ${context}`,
              },
              ...messages,
            ],
            temperature: 0.7,
  ...
  safeHandle("library:export-dossier", async (event, { name, items }) => {
    try {
      ...
      markdown += `*Generated by Writella Research Studio on ${new Date().toLocaleDateString()}*\n`;
  ...
  safeHandle("ai:reconstruct-transcript", async (_e, text) => {
    ...
    try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a script editor. Take the raw transcript and add proper punctuation and sensible paragraph breaks. IMPORTANT: Return the exact same text, just punctuated. Do not add summaries or intros.",
              },
      ...
      throw new Error(`Reconstruction failed: ${e.message}`);
    }
  });
  ...
  safeHandle("ai:processTranscript", async (_e, { text, prompt }) => {
    ...
    try {
        ...
        "https://api.deepseek.com/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: "English teacher. Analyze transcript.",
              },
      ...
      console.error("[processTranscript]", e.message);
  ...
  ipcMain.handle("ai:explain", async (_e, { text, videoTitle }) => {
    ...
    try {
        ...
        "https://api.deepseek.com/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: `Dictionary. 1-8 Format. Plain Text.`,
              },
              { role: "user", content: text },
            ],
          }),
      ...
      return {
        text,
        videoTitle,
        definition: "AI analysis timed out. Retry.",
        date: new Date().toISOString(),
      };
    }
  });
  ...
  const sendUStatus = (status, info = null) => {
    if (mainWindow)
      sendToRenderer(mainWindow.webContents, "update:status", { status, info });
  };
  ...
  autoUpdater.on("update-not-available", (info) =>
    sendUStatus("not-available", info),
  );
...
app.on("web-contents-created", (event, contents) => {
  contents.on("context-menu", (event, params) => {
    ...
    if (params.editFlags.canCopy)
      menu.append(new MenuItem({ label: "Copy Protocol", role: "copy" }));
    if (params.editFlags.canPaste)
      menu.append(new MenuItem({ label: "Paste Data", role: "paste" }));
    if (params.editFlags.canCut)
      menu.append(new MenuItem({ label: "Cut Block", role: "cut" }));

    if (params.editFlags.canSelectAll) {
      menu.append(new MenuItem({ type: "separator" }));
      menu.append(
        new MenuItem({ label: "Select All Nodes", role: "selectAll" }),
      );
    }
    ...
    if (params.isEditable) {
      ...
      menu.append(
        new MenuItem({
          label: "Neural Forge: Polish Draft",
          click: () => {
