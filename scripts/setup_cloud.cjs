...
console.log('--- StudyTube Neural Configurator ---');
...
try {
    ...
    // 3. Persist to SQLite
    db.prepare('INSERT OR REPLACE INTO settings (id, config) VALUES (1, ?)').run(JSON.stringify(config));
    
    console.log('--- CONFIGURATION COMPLETE ---');
    console.log('✅ Your local research is now connected to the Cloud Bridge.');
    console.log('Please restart your StudyTube app to begin synchronization.');
} catch (err) {
